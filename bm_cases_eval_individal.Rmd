...
# tests whether he rides bike 5 times per week (perhaps on the planned weekdays)



# go into schedule-level detail...
...
metrics.p13 <- merge(metrics.mob.p13,
                     metrics.act.p13,
                     by=c("dates", "participant"),
                     all=T)

T.move.pfunc <- ecdf(metrics.p13$Tt.move)
steps.pfunc <- ecdf(metrics.p13$steps)



# Tabulate results by week
weekly.results.p13 <- metrics.p13 %>% group_by(week) %>%
  summarise( Ndays = n(),
             move.days = sum(N.moves > 0),
             T.move.ave = round(mean(Tt.move*60)),
             T.active.ave = round(mean(active.time)),
             steps = round(mean(steps)),
             date = max(dates)) %>%
  mutate(SR.goal = c(perceived,NA),
         steps.prb = steps.pfunc(steps),
         T.move.prb = T.move.pfunc(T.move.ave/60))

weekly.results.p13[1:8,] %>% select(week, move.days,T.move.ave,T.move.prb,T.active.ave,steps,steps.prb,SR.goal)

plot_ly(weekly.results.p13[1:8,],
        x = ~week,
        y = ~SR.goal,
        type = "scatter",
        mode = "lines+markers") %>%
...
remove(metrics.p13, weekly.results.p13, T.move.pfunc, steps.pfunc, perceived)
