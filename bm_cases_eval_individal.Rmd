...
TO DO: replace scoring with edcf function (as in goal attainment)
...
p08.weekly.results <- p08.goals %>% filter(week < 9) %>%
  ...
  summarise(dates = max(dates),
            ...
            bike.time = (sum(total.time.bike)/sum(N.bike>0)) %>% round(2),
            vehicle.time = (sum(total.time.vehicle)/sum(N.vehicle>0)) %>% round(2)) %>%
  mutate(SR.goal = p08.perceived)

p08.weekly.results %>% select(week, bike.days, vehicle.days, bike.time, vehicle.time, SR.goal)
...
plot_ly(data = p10.goals,
  ...
  add_text(data = p10.SR, 
           ...
           textfont = list(color = "darkred", size = 12))
